...
export default function ResetPasswordPage(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    api.get<any>(`/auth/verify-reset-token?token=${token}`)
      .then(() => {
        setTokenValid(true)
      })
      .catch((err) => {
    ...
    <div className="relative min-h-screen bg-[#08080E] flex items-center justify-center overflow-hidden">
      ...
      >
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-xs text-white/40">Set up your new password details</p>
        </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-8 backdrop-blur-sm">
          {success ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                {submitting ? <Loader2 className="mx-auto h-5 w-5 animate-spin" /> : 'Reset Password'}
              </button>
            </form>
          )}
        </div>
